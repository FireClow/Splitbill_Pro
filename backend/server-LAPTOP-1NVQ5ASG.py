...
try:
    ...
    # Keep startup resilient if dotenv is not installed; environment variables can still come from OS.
...
def calculate_splits(bill: dict, method: str = "equal", custom_splits: dict = None, percentages: dict = None) -> list:
    ...
    if method == "equal":
        n = len(participants)
        # Split using integer cents to guarantee exact 2-decimal totals with no drift.
        ...
        for i, p in enumerate(participants):
            amount_cents = base_cents + (1 if i < remainder_cents else 0)
        ...
        if not percentages:
            raise ValueError("Percentage split requires percentages for participants")

        provided_ids = set(percentages.keys())
        unknown_ids = provided_ids - participant_ids
        if unknown_ids:
            raise ValueError("Percentage split contains invalid participant_id")

        total_percentage = Decimal("0")
        for p in participants:
            pct = Decimal(str(percentages.get(p["participant_id"], 0)))
            if pct < 0:
                raise ValueError("Percentage values must be greater than or equal to 0")
            total_percentage += pct

        if total_percentage.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP) != Decimal("100.00"):
            raise ValueError("Percentage split must sum to exactly 100")
        ...
        if not custom_splits:
            raise ValueError("Custom split requires amount values for participants")

        provided_ids = set(custom_splits.keys())
        unknown_ids = provided_ids - participant_ids
        if unknown_ids:
            raise ValueError("Custom split contains invalid participant_id")

        total_custom = Decimal("0")
        for p in participants:
            raw_amount = Decimal(str(custom_splits.get(p["participant_id"], 0)))
            if raw_amount < 0:
                raise ValueError("Custom split amounts must be greater than or equal to 0")
            total_custom += raw_amount

        total_custom = total_custom.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        expected_total = total.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        if total_custom != expected_total:
            raise ValueError("Custom split total must equal bill total")
...
async def update_payment(bill_id: str, participant_id: str, data: PaymentUpdate, user: dict = Depends(get_current_user)):
    ...
    target_split = next((s for s in splits if s.get("participant_id") == participant_id), None)
    if not target_split:
        raise HTTPException(status_code=400, detail="Invalid participant_id for this bill")

    amount_due = Decimal(str(target_split.get("amount_due", 0))).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    amount_paid = Decimal(str(data.amount_paid)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    if amount_paid > amount_due:
        raise HTTPException(status_code=400, detail="Amount paid cannot be greater than amount due")

    for s in splits:
        if s["participant_id"] == participant_id:
            s["amount_paid"] = float(amount_paid)
...
async def request_validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    message = first_error.get("msg", "Invalid request input")
    return JSONResponse(
        status_code=422,
        content={
            "error": {
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    logger.error(f"Unhandled server error: {type(exc).__name__}: {exc}", exc_info=True)
    return JSONResponse(
        status_code=500,
        content={
            "error": {
